...
function App() {
  ...
  const [moneyPyramid,setMoney]=useState(money)
  useEffect(() => {

    const featchQuestion = async ()=>{
      try {
        const res = await axios.get("/questions/");
        setQuestion(res.data.questions_data);
        const formattedData = res.data.prizes_data.map((item) => ({
          id: item.id[0], 
          amount: `$ ${item.amount[0]}` 
        })).reverse();

        setMoney(formattedData);
